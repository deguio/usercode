...
while (<LISTOFSamples>)
{
  ...
  ($sample,$dummy,$dataFlag,$isDD,$mW,$crossSection,$color) = split(" ") ;
  ...
  system ("rfdir ".$INPUTSAVEPath.$sample." | grep -v \"MiBiCommonNT.root\" | grep -v \"histo_WprimeTreeAnalysis.root\" | awk '{print \$9}' > ".$LISTOFFiles."\n") ;
  ...
  { 
    ...
    system ("cat ".$JOBCfgTemplate."   | sed -e s%MASS%".$mW.
                                   ...
                                   "%g | sed -e s%OUTPUTFILENAME%".$OUTPUTFILEName."_".$jobIt.".root".
                                   "%g > ".$tempo1) ;
    ...
    $command = "cd /afs/cern.ch/user/d/deguio/scratch0/Wprime/MiBiCommonPAT/CMSSW_4_2_3/src/" ;
    print SAMPLEJOBFILE $command."\n";
    ...
    $command = "bsub -q 1nd -cwd ".$jobDir." ".$tempBjob."\n" ;  
